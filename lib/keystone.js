...
//gets a list of all domains in the system
//calls back with cb(error, domains_array)
//***NOTE: admin_access_token is a scoped token from a project you have admin rights on - yes this is weird
Keystone.prototype.listDomains = function(admin_access_token, cb)
{
  var self = this;
  var request_options = this.getRequestOptions(admin_access_token, '/domains', true);
  request_options.metricPath = 'remote-calls.keystone.projects.list';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'domains';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    for(var n = 0; n < body.domains.length; n++)
    {
      domains_array[n] = self.mangleObject('Project', body.domains[n]);
    }

    //tack these on for easy consupmtion and in case we ever need pagination
    domains_array.self = body.links.self;
    domains_array.previous = body.links.previous;
    domains_array.next = body.links.next;

    cb(null, domains_array);
  });
};
